...
## Purpose
This wiki is a structured, interlinked knowledge base for Income Tax domain.
Claude maintains the wiki. The human curates sources, asks questions, and guides the analysis.

## Folder structure
```
raw/               -- source documents (immutable -- never modify these)
raw/lecp/          -- original LECP zip files (immutable)
raw/extracted/     -- JSON files extracted from LECP zips (immutable)
project/           -- LECP files created by Claude (prefix: proj-)
...
LECP files are zip archives containing JSON data.

**Ingestion** (user provides LECP file):
1. Keep original zip in `raw/lecp/`
2. Unzip and extract JSON to `raw/extracted/`
3. Never modify files in `raw/lecp/` or `raw/extracted/`
...
1. If source is LECP: follow LECP ingestion steps above first, then read the extracted JSON
